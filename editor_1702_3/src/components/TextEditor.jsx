...
const TextEditor = ({printer,label,role, value,styles,onStyleChange, 
  ...
  // width:'138.5vw'
  // console.log(printer)
  // console.log(parseInt(styles.width) * 1.73)
  return (
    <div>
      
      <div className='relative p-2 items-center'>
        {switch1==false ? 
        label == '' ?  (<div onClick={()=>{onLineSelected()}} style= {{...styles , width: printer==true ?  `${parseInt(styles.width) * 1.73}vw` : styles.width  }}></div>): 
        (<div
        style={{
          // ...textStyle, 
      ...
      {lineFocus && <div id='display-none' className='absolute -top-2 -right-2 p-2'>
        <button onMouseDown={(e)=>{
          // console.log('clicked!!');
          onDeleteClick(e, coreIndex,type);
        }}  className='bg-black rounded-full text-white'>
